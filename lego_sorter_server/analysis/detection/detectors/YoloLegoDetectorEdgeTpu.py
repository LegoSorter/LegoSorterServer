...
class YoloLegoDetectorEdgeTpu(LegoDetector, metaclass=ThreadSafeSingleton):
    ...
    def convert_results_to_common_format(results) -> DetectionResults:
        ...
        boxes = YoloLegoDetectorEdgeTpu.xyxy2yxyx_scaled(image_predictions[:, :4])
    ...
    # pass photo scaled to 570x320 or padded 570x570
    def detect_lego(self, image: numpy.ndarray) -> DetectionResults:
        ...
        cropedTop = image[0:320, :].copy()
        cropedMiddle = image[125:445, :].copy()
        cropedBottom = image[250:570, :].copy()

        # cropedTop = image[0:320, 0:320].copy()
        # cropedMiddle = image[160:480, 0:320].copy()
        # cropedBottom = image[320:640, 0:320].copy()

        # cv2.imwrite("/home/adam/lego/cv2_test/LegoSorterServer/11.jpg", cropedTop)
        # cv2.imwrite("/home/adam/lego/cv2_test/LegoSorterServer/22.jpg", cropedMiddle)
        # cv2.imwrite("/home/adam/lego/cv2_test/LegoSorterServer/33.jpg", cropedBottom)
        ...
        result1 = result1[0]
        result1[:, [1, 3]] = result1[:, [1, 3]]/(570/320)
        ...
        result2 = result2[0]
        result2[:, [1, 3]] = (result2[:, [1, 3]] / (570/320))  # scale in half y coordinate and height
        result2[:, 1] = result2[:, 1] + (125/570)  # move y coordinate
        ...
        result3 = result3[0]
        result3[:, [1, 3]] = (result3[:, [1, 3]] / (570/320))
        result3[:, 1] = result3[:, 1] + (250/570)
        ...
        result = YoloLegoDetectorEdgeTpu.non_max_suppression(result, self.conf_thresh, self.iou_thresh, self.filter_classes, self.agnostic_nms, max_det=self.max_det)
        # torchvision.ops.nms()
        # res=result[0]
    ...
    def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False,
        ...
        for xi, x in enumerate(prediction):  # image index, image inference
            ...
            box = YoloLegoDetectorEdgeTpu.xywh2xyxy(x[:, :4])
            ...
            i = YoloLegoDetectorEdgeTpu.nms(boxes, scores, iou_thres)  # NMS

            if i.shape[0] > max_det:  # limit detections
                i = i[:max_det]
            if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
                # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
                iou = YoloLegoDetectorEdgeTpu.box_iou(boxes[i], boxes) > iou_thres  # iou matrix
