...
class SortingProcessor:
    ...
    def process_next_image(self, image: Image, save_image: bool = True):
        ...
        logger.info(f"[SortingProcessor] Processing an image took {elapsed_ms} ms.")
        ...
        if save_image is True and len(current_results) > 0:
            ...
            logger.info(f"[SortingProcessor] Saving images took {1000 * (time.time() - start_time_saving)} ms.")
    ...
    def _send_results_to_controller(self):
        ...
        logger.info(f"[SortingProcessor] Got the best result {best_result}. Returning the results...")
    ...
    def _process(self, image: Image) -> List[Tuple]:
        ...
        logger.info(f"[SortingProcessor] Detected a lego brick, processing...")

        if detected_count > 1:
            logger.warning(f"[SortingProcessor] More than one brick detected '(detected_count = {detected_count}), "
